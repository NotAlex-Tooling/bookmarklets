...
// description: "Runs on a YouTube video or Short. Pulls the maximum-resolution thumbnail (maxresdefault down to default), the JPG and WebP variants, and the auto-generated start/middle/end storyboard frames — each with open, copy and download. Pure URL construction, no requests to Google, Trusted-Types-safe."
(function() {
  ...
  var MONO = '"SF Mono","JetBrains Mono","Fira Mono",Consolas,monospace';
    ...
    var TITLE = 'YouTube Thumbnail Grabber';

  var host = window.location.hostname.replace(/^www\./, '').replace(/^m\./, '');
  var onYT = /(?:^|\.)youtube\.com$/.test(host) || host === 'youtu.be';

  var vid = onYT ? getVideoId() : null;

  var CTX_BADGE = vid ? vid : (onYT ? 'no video on this page' : 'not on youtube');
  var WRONG_PAGE = onYT ? (vid ? null : 'Open a YouTube video or Short to grab its thumbnail.') : 'Open a YouTube video to use this.';

  var TABS_DEF = [
    { id: 'thumbs', label: 'Thumbnails', render: renderThumbs },
  ...
  switchTab(activeTab);
    ...
    // Extracts the 11-char video ID from the current YouTube URL (watch, short, embed, live, youtu.be).
  function getVideoId() {
    try {
    ...
    var m = p.match(/\/shorts\/([\w-]{11})/) || p.match(/\/embed\/([\w-]{11})/) || p.match(/\/live\/([\w-]{11})/);
    if (m) return m[1];
    if (/youtu\.be$/.test(window.location.hostname.replace(/^www\./, ''))) {
      var s = p.replace(/^\//, '').slice(0, 11);
      if (/^[\w-]{11}$/.test(s)) return s;
    }
    return null;
  }

  var thumbBase = 'https://i.ytimg.com/vi/' + vid + '/';
  var webpBase = 'https://i.ytimg.com/vi_webp/' + vid + '/';

  // Triggers a download of a cross-origin image URL.
  function downloadUrl(url, name) {
    var a = el('a'); a.href = url; a.download = name; a.target = '_blank';
    document.body.appendChild(a); a.click(); a.remove();
  }

  // Builds a thumbnail row: preview + label/dims + url + action buttons; hides preview if the size 404s.
  function thumbRow(label, url, dims, dlName) {
    var r = el('div', 'padding:11px 0;border-bottom:1px solid ' + COLORS.borderSoft + ';display:flex;gap:12px;align-items:flex-start');
    var t = el('img', 'width:96px;height:54px;object-fit:cover;border-radius:4px;border:1px solid ' + COLORS.border + ';background:#000;flex-shrink:0');
    t.referrerPolicy = 'no-referrer'; t.src = url;
    var info = el('div', 'flex:1;min-width:0');
    var head = el('div', 'font-size:12px;font-weight:600;color:' + COLORS.textStrong, label + '  \u00b7  ' + dims);
    info.appendChild(head);
    info.appendChild(el('div', 'font-family:' + MONO + ';font-size:10px;color:' + COLORS.textDim + ';word-break:break-all;margin:3px 0 7px 0', url));
    var btns = el('div', 'display:flex;gap:6px;flex-wrap:wrap');
    btns.appendChild(btn('Open', function() { window.open(url, '_blank'); }));
    btns.appendChild(btn('Copy', function() { copyText(url); }));
    btns.appendChild(btn('Download', function() { downloadUrl(url, dlName); }));
    info.appendChild(btns);
    t.onerror = function() { t.style.display = 'none'; head.textContent = label + '  \u00b7  not available for this video'; head.style.color = COLORS.textDim; };
  ...
  function renderThumbs(container) {
    container.appendChild(sectionTitle('Best available'));
    container.appendChild(noteBanner('anonymous', 'Passive', 'Thumbnail URLs are constructed from the video ID \u2014 nothing is requested from Google by this tool, and the uploader is not notified.'));

    var chain = [thumbBase + 'maxresdefault.jpg', thumbBase + 'sddefault.jpg', thumbBase + 'hqdefault.jpg'];
    var idx = 0;
    var preview = el('div', 'padding:6px 0 10px 0;text-align:center');
    var img = el('img', 'max-width:100%;border-radius:10px;border:1px solid ' + COLORS.border + ';box-shadow:0 6px 24px rgba(0,0,0,0.5);background:#000');
    img.referrerPolicy = 'no-referrer'; img.src = chain[0];
    var cap = el('div', 'color:' + COLORS.textDim + ';font-size:10.5px;font-family:' + MONO + ';margin-top:6px', 'maxresdefault.jpg (1280\u00d7720)');
    img.onerror = function() { idx++; if (idx < chain.length) { img.src = chain[idx]; cap.textContent = chain[idx].split('/').pop() + ' (maxres unavailable, showing fallback)'; } };
    preview.appendChild(img); preview.appendChild(cap);
    container.appendChild(preview);
    var act = el('div', 'display:flex;gap:8px;flex-wrap:wrap;padding-bottom:4px');
    act.appendChild(btn('Open maxres', function() { window.open(thumbBase + 'maxresdefault.jpg', '_blank'); }, { primary: true }));
    act.appendChild(btn('Download maxres', function() { downloadUrl(thumbBase + 'maxresdefault.jpg', vid + '_maxres.jpg'); }));
    container.appendChild(act);

    container.appendChild(sectionTitle('All JPG resolutions'));
    var sizes = [
      ['maxresdefault.jpg', 'maxres', '1280\u00d7720'],
      ['sddefault.jpg', 'sd', '640\u00d7480'],
      ['hqdefault.jpg', 'hq', '480\u00d7360'],
      ['mqdefault.jpg', 'mq', '320\u00d7180'],
      ['default.jpg', 'default', '120\u00d790']
    ];
    var rows = [];
    for (var i = 0; i < sizes.length; i++) {
      var r = thumbRow(sizes[i][1], thumbBase + sizes[i][0], sizes[i][2], vid + '_' + sizes[i][1] + '.jpg');
      container.appendChild(r); rows.push(r);
    }
    stagger(rows, 30, 40);

    container.appendChild(sectionTitle('WebP (smaller, same frame)'));
    var wrows = [];
    var wr = thumbRow('maxres webp', webpBase + 'maxresdefault.webp', '1280\u00d7720', vid + '_maxres.webp');
    container.appendChild(wr); wrows.push(wr);
    var wr2 = thumbRow('hq webp', webpBase + 'hqdefault.webp', '480\u00d7360', vid + '_hq.webp');
    container.appendChild(wr2); wrows.push(wr2);
    stagger(wrows, 30, 60);
  }

  // Renders the Frames (storyboard) tab.
  function renderFrames(container) {
    container.appendChild(sectionTitle('Auto-generated frames'));
    container.appendChild(el('div', 'color:' + COLORS.text + ';font-size:11.5px;padding:2px 0 10px 0;line-height:1.5', 'YouTube samples three frames from across the video plus a poster. Useful when the chosen thumbnail hides context.'));
    var frames = [
      ['0.jpg', 'Poster (0)'],
      ['1.jpg', 'Start (~25%)'],
      ['2.jpg', 'Middle (~50%)'],
    ...
    var grid = el('div', 'display:grid;grid-template-columns:1fr 1fr;gap:12px;padding-top:4px');
    var cells = [];
    for (var i = 0; i < frames.length; i++) {
      (function(file, label) {
        var url = thumbBase + file;
        var cell = el('div', 'border:1px solid ' + COLORS.borderSoft + ';border-radius:8px;overflow:hidden;background:' + COLORS.surface);
        var im = el('img', 'width:100%;display:block;aspect-ratio:4/3;object-fit:cover;background:#000;cursor:pointer');
        im.referrerPolicy = 'no-referrer'; im.src = url;
        im.onclick = function() { window.open(url, '_blank'); };
        im.title = 'Open full size';
        cell.appendChild(im);
        var lab = el('div', 'display:flex;justify-content:space-between;align-items:center;padding:7px 9px;gap:6px');
        lab.appendChild(el('span', 'font-size:11px;font-weight:600;color:' + COLORS.textStrong, label));
        var cp = el('span', 'font-size:10px;color:' + COLORS.accent + ';cursor:pointer;font-family:' + MONO, 'copy');
  ...
  function renderPivot(container) {
    ...
    container.appendChild(row('Video ID', vid, { copy: true, mono: true }));
    container.appendChild(linkRow('Watch URL', 'https://www.youtube.com/watch?v=' + vid));

    container.appendChild(sectionTitle('Dig deeper'));
    pivotList(container, [
      { name: 'YTGlimpse', desc: 'Thumbnail & frame inspection', url: 'https://ytglimpse.notalex.sh' },
      { name: 'Filmot', desc: 'Search this video\u2019s captions/subtitles', url: 'https://filmot.com/video/' + vid },
      { name: 'Google reverse image', desc: 'Trace where this thumbnail appears', url: 'https://lens.google.com/uploadbyurl?url=' + encodeURIComponent(thumbBase + 'maxresdefault.jpg') },
      { name: 'Wayback Machine', desc: 'Historical snapshots of this video page', url: 'https://web.archive.org/web/*/youtube.com/watch?v=' + vid }
    ]);
  }
