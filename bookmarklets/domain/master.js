...
// description: "Runs on any page. Resolves the current domain over DNS-over-HTTPS (A/AAAA/MX/NS/TXT), pulls subdomains from crt.sh certificate transparency, sprays the base name across common TLDs, and deep-links to RDAP/WHOIS, Shodan, urlscan, VirusTotal and Wayback. Live lookups fall back to link-outs when a page's security policy blocks them."
(function() {
  ...
  var MONO = '"SF Mono","JetBrains Mono","Fira Mono",Consolas,monospace';
    ...
    var TITLE = 'Domain & Infra Recon';

  var fullHost = window.location.hostname || '';
  var isIP = /^[0-9.]+$/.test(fullHost) || fullHost.indexOf(':') >= 0;

  // Reduces a hostname to its registrable domain, honouring common 2-level suffixes.
  function registrable(h) {
    var parts = h.split('.');
    if (parts.length <= 2) return h;
    var two = ['co.uk', 'org.uk', 'gov.uk', 'ac.uk', 'com.au', 'net.au', 'org.au', 'co.nz', 'co.jp', 'co.in', 'com.br', 'com.cn', 'co.za', 'com.mx', 'com.tr', 'co.kr', 'com.sg', 'com.hk'];
    var last2 = parts.slice(-2).join('.');
    if (two.indexOf(last2) >= 0) return parts.slice(-3).join('.');
    return last2;
  }

  var regDomain = isIP ? fullHost : registrable(fullHost);
  var baseLabel = regDomain.split('.')[0];

  var CTX_BADGE = fullHost || 'no host';
  var WRONG_PAGE = (!fullHost || fullHost === 'localhost') ? 'This page has no public domain to inspect (' + (fullHost || 'about:blank') + ').' : null;

  var TABS_DEF = [
    { id: 'overview', label: 'Overview',  render: renderOverview },
    { id: 'dns',      label: 'DNS',       render: renderDNS },
    { id: 'certs',    label: 'Certs',     render: renderCerts },
    { id: 'tld',      label: 'TLD spray', render: renderTLD },
  ...
  switchTab(activeTab);
    ...
    // Queries Cloudflare DNS-over-HTTPS for a record type; returns the Answer array.
  function doh(name, type) {
    return fetch('https://cloudflare-dns.com/dns-query?name=' + encodeURIComponent(name) + '&type=' + type, { headers: { accept: 'application/dns-json' } })
      .then(function(r) { if (!r.ok) throw new Error('DoH HTTP ' + r.status); return r.json(); })
      .then(function(j) { return (j && j.Answer) ? j.Answer : []; });
  }

  // Renders a small note explaining a blocked cross-origin request, with a fallback link.
  function blockedNote(container, label, url) {
    container.appendChild(statusBox('Live lookup blocked by this page\u2019s security policy (CSP/CORS). Open it directly instead:', false));
    container.appendChild(pivotCard(label, url, url));
  }

  // Renders the Overview tab.
  function renderOverview(container) {
    container.appendChild(sectionTitle('Target'));
    container.appendChild(row('Hostname', fullHost, { copy: true, mono: true }));
    if (!isIP) container.appendChild(row('Registrable', regDomain, { copy: true, mono: true }));
    if (!isIP) container.appendChild(row('TLD', regDomain.split('.').slice(1).join('.'), { mono: true }));
    container.appendChild(row('Protocol', window.location.protocol.replace(':', '')));
    if (document.title) container.appendChild(row('Page title', document.title));

    container.appendChild(sectionTitle('Resolves to'));
    asyncMount(container, 'Resolving over DoH\u2026', function(c) {
      return Promise.all([doh(fullHost, 'A').catch(function() { return []; }), doh(fullHost, 'AAAA').catch(function() { return []; })])
        .then(function(res) {
          var a = res[0], aaaa = res[1];
          var any = false;
          for (var i = 0; i < a.length; i++) { if (a[i].type === 1) { c.appendChild(row('IPv4', a[i].data, { copy: true, mono: true })); any = true; } }
          for (var j = 0; j < aaaa.length; j++) { if (aaaa[j].type === 28) { c.appendChild(row('IPv6', aaaa[j].data, { copy: true, mono: true })); any = true; } }
          if (!any) c.appendChild(statusBox('No A/AAAA records returned (or the request was blocked by this page\u2019s CSP). Try the Pivot tab.', false));
        });
    });
  }

  // Renders the DNS tab with multiple record types.
  function renderDNS(container) {
    container.appendChild(sectionTitle('DNS records \u00b7 ' + regDomain));
    container.appendChild(noteBanner('login', 'Active', 'Queries Cloudflare DoH from your browser. Some sites\u2019 CSP will block this \u2014 the Pivot tab always works.'));
    var types = [['A', 1], ['AAAA', 28], ['MX', 15], ['NS', 2], ['TXT', 16], ['CNAME', 5], ['SOA', 6]];
    asyncMount(container, 'Querying ' + types.length + ' record types\u2026', function(c) {
      var chain = Promise.resolve();
      var gotAny = { v: false };
      types.forEach(function(t) {
        chain = chain.then(function() {
          return doh(regDomain, t[1]).then(function(ans) {
            var recs = ans.filter(function(r) { return r.type === t[1]; });
            if (!recs.length) return;
            gotAny.v = true;
            c.appendChild(sectionTitle(t[0]));
            for (var i = 0; i < recs.length; i++) {
              c.appendChild(el('div', 'padding:7px 0;border-bottom:1px solid ' + COLORS.borderSoft + ';font-family:' + MONO + ';font-size:11.5px;color:' + COLORS.textStrong + ';word-break:break-all', recs[i].data));
            }
          }).catch(function() {});
        });
      });
      return chain.then(function() {
        if (!gotAny.v) c.appendChild(statusBox('No records returned. The page\u2019s CSP likely blocked the DoH request \u2014 use the Pivot tab.', true));
      });
    });
  }

  // Renders the certificate-transparency (crt.sh) tab.
  function renderCerts(container) {
    container.appendChild(sectionTitle('Certificate transparency'));
    container.appendChild(el('div', 'color:' + COLORS.text + ';font-size:11.5px;padding:2px 0 8px 0;line-height:1.5', 'Subdomains seen in issued TLS certificates for ' + regDomain + ' (via crt.sh).'));
    var crtUrl = 'https://crt.sh/?q=%25.' + encodeURIComponent(regDomain) + '&output=json';
    asyncMount(container, 'Fetching crt.sh\u2026', function(c) {
      return fetch(crtUrl).then(function(r) { if (!r.ok) throw new Error('crt.sh HTTP ' + r.status); return r.json(); }).then(function(rows) {
        var names = {};
        for (var i = 0; i < rows.length; i++) {
          var nv = (rows[i].name_value || '').split('\n');
          for (var k = 0; k < nv.length; k++) {
            var n = nv[k].trim().toLowerCase();
            if (n && n.indexOf('*') !== 0) names[n] = true;
          }
        }
        var list = Object.keys(names).sort();
        c.appendChild(row('Unique names', String(list.length), { mono: true }));
        var cap = list.slice(0, 60);
        var nodes = [];
        for (var j = 0; j < cap.length; j++) {
          var a = el('a', 'display:block;padding:6px 0;border-bottom:1px solid ' + COLORS.borderSoft + ';font-family:' + MONO + ';font-size:11px;color:' + COLORS.textStrong + ';text-decoration:none;word-break:break-all', cap[j]);
          a.href = 'https://' + cap[j]; a.target = '_blank'; a.rel = 'noopener noreferrer';
          c.appendChild(a); nodes.push(a);
        }
        if (list.length > cap.length) c.appendChild(el('div', 'padding:8px 0;color:' + COLORS.textDim + ';font-size:11px', '+ ' + (list.length - cap.length) + ' more \u2014 open crt.sh for the full list.'));
        stagger(nodes, 18, 40);
      }).catch(function() {
        blockedNote(c, 'crt.sh \u2014 ' + regDomain, 'https://crt.sh/?q=%25.' + encodeURIComponent(regDomain));
      });
    });
  }

  // Renders the TLD-spray tab (resolves the base label across many TLDs on demand).
  function renderTLD(container) {
    container.appendChild(sectionTitle('TLD spray \u00b7 ' + baseLabel));
    container.appendChild(el('div', 'color:' + COLORS.text + ';font-size:11.5px;padding:2px 0 10px 0;line-height:1.5', 'Resolves "' + baseLabel + '" across common TLDs to find sibling/typo domains. This fires one DoH lookup per TLD.'));
    var tlds = ['com', 'net', 'org', 'io', 'co', 'app', 'dev', 'ai', 'me', 'info', 'biz', 'xyz', 'online', 'site', 'tech', 'store', 'us', 'uk', 'ca', 'de', 'fr', 'nl', 'es', 'it', 'ru', 'jp', 'cn', 'in', 'br', 'tv', 'cc', 'gg', 'sh', 'so'];
    var result = el('div', 'margin-top:4px');
    var go = btn('Resolve ' + baseLabel + '.* (' + tlds.length + ' TLDs)', function() { run(); }, { primary: true });
    container.appendChild(go);
    container.appendChild(result);

    // Resolves the base label across every candidate TLD and lists those that have DNS records.
    function run() {
      go.disabled = true; go.style.opacity = '0.5';
      clearNode(result);
      var sp = spinner('Checking ' + tlds.length + ' TLDs\u2026');
      result.appendChild(sp);
      var hits = el('div'), live = 0, done = 0, blocked = 0;
      var chain = Promise.resolve();
      tlds.forEach(function(tld) {
        var fqdn = baseLabel + '.' + tld;
        chain = chain.then(function() {
          return doh(fqdn, 'A').then(function(ans) {
            done++;
            var ips = ans.filter(function(r) { return r.type === 1; });
            if (ips.length) {
              live++;
              var r = el('div', 'padding:7px 0;border-bottom:1px solid ' + COLORS.borderSoft + ';display:flex;gap:12px;align-items:baseline');
              var dn = el('a', 'color:' + COLORS.accent + ';font-family:' + MONO + ';font-size:11.5px;text-decoration:none;min-width:150px', fqdn);
              dn.href = 'https://' + fqdn; dn.target = '_blank'; dn.rel = 'noopener noreferrer';
              r.appendChild(dn);
              r.appendChild(el('span', 'color:' + COLORS.textMuted + ';font-family:' + MONO + ';font-size:11px', ips[0].data));
              hits.appendChild(r);
            }
      ...
      chain.then(function() {
        if (sp.parentNode) sp.remove();
        if (blocked >= tlds.length) { result.appendChild(statusBox('All lookups were blocked by this page\u2019s CSP. Run this bookmarklet on a less restrictive page, or use the Pivot tab.', true)); }
        else { result.appendChild(row('Resolved', live + ' / ' + done, { mono: true })); result.appendChild(hits); }
        go.disabled = false; go.style.opacity = '1';
      });
    }
  }

  // Renders the Pivot tab with WHOIS/cert/infra link-outs.
  function renderPivot(container) {
    container.appendChild(sectionTitle('Registration & ownership'));
    pivotList(container, [
      { name: 'ICANN Lookup', desc: 'Official WHOIS/RDAP record', url: 'https://lookup.icann.org/en/lookup?name=' + encodeURIComponent(regDomain) },
      { name: 'RDAP.org', desc: 'Structured registration data (JSON)', url: 'https://rdap.org/domain/' + encodeURIComponent(regDomain) },
      { name: 'Whoxy history', desc: 'Historical WHOIS / past owners', url: 'https://www.whoxy.com/' + encodeURIComponent(regDomain) + '#history' }
    ]);
    container.appendChild(sectionTitle('Certificates & subdomains'));
    pivotList(container, [
      { name: 'crt.sh', desc: 'All issued TLS certificates', url: 'https://crt.sh/?q=%25.' + encodeURIComponent(regDomain) },
      { name: 'Censys', desc: 'Hosts & certs for this domain', url: 'https://search.censys.io/search?resource=hosts&q=' + encodeURIComponent(regDomain) }
    ]);
    container.appendChild(sectionTitle('Infrastructure & reputation'));
    pivotList(container, [
      { name: 'Shodan', desc: 'Exposed services & banners', url: 'https://www.shodan.io/search?query=hostname%3A' + encodeURIComponent(regDomain) },
      { name: 'urlscan.io', desc: 'Page scans, redirects, resources', url: 'https://urlscan.io/search/#' + encodeURIComponent(regDomain) },
      { name: 'VirusTotal', desc: 'Reputation, resolutions, detections', url: 'https://www.virustotal.com/gui/domain/' + encodeURIComponent(regDomain) },
      { name: 'SecurityTrails', desc: 'DNS history & subdomains', url: 'https://securitytrails.com/domain/' + encodeURIComponent(regDomain) + '/dns' },
      { name: 'DNSlytics', desc: 'Reverse IP / analytics / ASN', url: 'https://dnslytics.com/domain/' + encodeURIComponent(regDomain) }
    ]);
    container.appendChild(sectionTitle('Archive'));
    pivotList(container, [
      { name: 'Wayback Machine', desc: 'Historical snapshots of this site', url: 'https://web.archive.org/web/*/' + encodeURIComponent(regDomain) }
    ]);
  }
