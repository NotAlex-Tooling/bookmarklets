// title: "Snapchat OSINT"
// description: "Runs on a public Snapchat profile (snapchat.com/add/<name>). Reads username, display name, subscriber count, snapcode and bio from the embedded __NEXT_DATA__ (passive), enumerates historical Bitmoji outfits from the avatar CDN (active), and best-effort extracts Spotlight/Story tile IDs. Pivots to BACKMOJI and WhatsMyName."
(function() {
  ...
  var MONO = '"SF Mono","JetBrains Mono","Fira Mono",Consolas,monospace';
    ...
    var TITLE = 'Snapchat OSINT';

  var host = window.location.hostname.replace(/^www\./, '');
  var onSnap = /(?:^|\.)snapchat\.com$/.test(host);
  var um = window.location.pathname.match(/\/(?:add|@)([A-Za-z0-9._-]+)/);
  var username = (onSnap && um) ? um[1] : null;

  var CTX_BADGE = onSnap ? (username ? '@' + username : 'snapchat') : 'not on snapchat';
  var WRONG_PAGE = onSnap ? null : 'Open a public Snapchat profile (snapchat.com/add/<name>) to use this.';

  var TABS_DEF = [
    { id: 'about',   label: 'About',      render: renderAbout },
    { id: 'bitmoji', label: 'Bitmoji',    render: renderBitmoji },
    { id: 'time',    label: 'Timestamps', render: renderTime },
  ...
  switchTab(activeTab);
    ...
    // Parses the Next.js __NEXT_DATA__ script block into an object.
  function getNextData() {
    try { var e = document.querySelector('script#__NEXT_DATA__'); if (!e) return null; return JSON.parse(e.textContent); } catch (x) { return null; }
  }

  // Returns props.pageProps from __NEXT_DATA__ if present.
  function pageProps() {
    var n = getNextData();
    return (n && n.props && n.props.pageProps) ? n.props.pageProps : null;
  }

  // Reads an Open Graph / meta tag value.
  function metaTag(p) {
  ...
  function renderAbout(container) {
    container.appendChild(noteBanner('anonymous', 'Passive', 'Reads the profile JSON Snapchat embeds in the page (__NEXT_DATA__). No requests are sent and the user is not notified.'));
    var pp = pageProps();
    var info = null;
    if (pp) {
      info = (pp.userProfile && (pp.userProfile.publicProfileInfo || pp.userProfile.userInfo)) || pp.publicProfileInfo || pp.userProfile || null;
    }

    container.appendChild(sectionTitle('Identity'));
    var uname = (info && info.username) || username;
    container.appendChild(row('Username', uname ? '@' + uname : '\u2014', { copy: !!uname }));
    var disp = (info && (info.displayName || info.title)) || metaTag('og:title');
    if (disp) container.appendChild(row('Display name', disp, { copy: true }));
    var subs = info && (info.subscriberCount != null ? info.subscriberCount : info.subscriberCountValue);
    if (subs != null) container.appendChild(row('Subscribers', Number(subs).toLocaleString('en-US'), { mono: true }));
    if (username) container.appendChild(linkRow('Profile URL', 'https://www.snapchat.com/add/' + username));

    var snap = info && (info.snapcodeImageUrl || info.snapcodeUrl);
    if (snap) {
      container.appendChild(sectionTitle('Snapcode'));
      container.appendChild(previewImage(snap, { maxHeight: '170px' }));
      container.appendChild(urlBox(snap));
    }

    var bio = (info && (info.bio || info.bioText)) || metaTag('og:description');
    if (bio) {
    ...
    if (!info && !disp) container.appendChild(statusBox('Could not find structured profile data. Snapchat may have changed its page format, or this is not a public profile.', true));
  }

  // Finds the current Bitmoji 3D render URL in the page HTML.
  function findRenderUrl() {
    var html = document.documentElement.outerHTML;
    var m = html.match(/https:\/\/cf-st\.sc-cdn\.net\/3d\/render\/[^"'\s\\]+/) || html.match(/https:\/\/images\.bitmoji\.com\/3d\/avatar\/[^"'\s\\]+/);
    return m ? m[0].replace(/\\u0026/g, '&') : null;
  }

  // Parses comic/avatar IDs, current index and scale out of a Bitmoji render URL.
  function parseBitmoji(url) {
    if (!url) return null;
    var m = url.match(/\/(\d+)-(\d+)_(\d+)-s(\d+)-v1\./);
    if (!m) return null;
    return { comicId: m[1], avatarId: m[2], index: parseInt(m[3], 10), scale: parseInt(m[4], 10) };
  }

  // Renders the Bitmoji enumeration tab.
  function renderBitmoji(container) {
    container.appendChild(sectionTitle('Bitmoji avatar history'));
    container.appendChild(noteBanner('login', 'Active', 'Enumeration loads many images from the Bitmoji CDN to surface older outfits. It does not notify the target, but it fires a burst of requests \u2014 run it deliberately.'));
    var url = findRenderUrl();
    var bm = parseBitmoji(url);
    if (!bm) {
      container.appendChild(statusBox('No current Bitmoji 3D render URL found on this page. The profile may have no Bitmoji, or Snapchat changed the format.', true));
      container.appendChild(sectionTitle('Fallback'));
      container.appendChild(pivotCard('BACKMOJI', 'Manual Bitmoji history tool (My OSINT Training)', 'https://backmoji.myosint.training'));
      return;
    }
    container.appendChild(row('Comic ID', bm.comicId, { mono: true, copy: true }));
    container.appendChild(row('Avatar ID', bm.avatarId, { mono: true, copy: true }));
    container.appendChild(row('Current index', String(bm.index), { mono: true }));
    container.appendChild(row('Scale', String(bm.scale), { mono: true }));
    container.appendChild(previewImage(url, { maxHeight: '150px' }));

    var maxN = bm.index;
    var go = btn('Enumerate ' + (maxN + 1) + ' outfits', function() { run(); }, { primary: true });
    container.appendChild(go);
    var grid = el('div', 'display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-top:10px');
    container.appendChild(grid);
    var status = el('div', 'padding:6px 0;color:' + COLORS.textDim + ';font-size:11px;font-family:' + MONO);
    container.appendChild(status);

    // Builds and appends one outfit thumbnail cell to the grid.
    function addCell(u, idx) {
      var cell = el('div', 'border:1px solid ' + COLORS.borderSoft + ';border-radius:8px;overflow:hidden;background:' + COLORS.surface + ';cursor:pointer');
      var im = el('img', 'width:100%;display:block;aspect-ratio:1/1;object-fit:cover;background:#000');
      im.referrerPolicy = 'no-referrer'; im.src = u;
      im.onclick = function() { window.open(u, '_blank'); };
      cell.appendChild(im);
      cell.appendChild(el('div', 'text-align:center;font-size:9.5px;color:' + COLORS.textDim + ';font-family:' + MONO + ';padding:3px 0', '#' + idx));
      grid.appendChild(cell);
      animateIn(cell, 0);
    }
    // Re-enables the button and writes the final outfit-count status.
    function finish(found) {
      go.disabled = false; go.style.opacity = '1';
      status.textContent = found === 0 ? 'No outfits resolved (the CDN format may have changed).' : (found + ' outfit' + (found === 1 ? '' : 's') + ' found \u00b7 lower # = older');
    }
    // Enumerates Bitmoji outfit images by index and renders each one that loads.
    function run() {
      go.disabled = true; go.style.opacity = '0.5'; clearNode(grid);
      var found = 0, done = 0, total = maxN + 1;
      status.textContent = 'Loading 0/' + total + '\u2026';
      for (var n = 0; n <= maxN; n++) {
        (function(idx) {
          var u = 'https://images.bitmoji.com/3d/avatar/' + bm.comicId + '-' + bm.avatarId + '_' + idx + '-s' + bm.scale + '-v1.webp';
          var im = new Image();
          im.onload = function() { done++; found++; addCell(u, idx); status.textContent = 'Loading ' + done + '/' + total + ' \u00b7 ' + found + ' found'; if (done === total) finish(found); };
          im.onerror = function() { done++; status.textContent = 'Loading ' + done + '/' + total + ' \u00b7 ' + found + ' found'; if (done === total) finish(found); };
          im.src = u;
        })(n);
      }
    }
  ...
  // Extracts a media ID from a Snapchat CDN src via the /d/<id> pattern.
  function mediaId(src) {
    try { var o = (src || '').match(/\/d\/([^/.?]+)/); return o ? o[1] : null; } catch (e) { return null; }
  }

  // Renders the best-effort Spotlight/Story timestamp tab.
  function renderTime(container) {
    container.appendChild(sectionTitle('Spotlight & Story tiles'));
    container.appendChild(noteBanner('login', 'Fragile', 'Snapchat changes this page structure frequently. This reads tiles currently in the DOM and surfaces any media IDs found \u2014 it may return nothing even when content is visible. Treat results as best-effort.'));

    var spot = document.querySelectorAll('[data-testid="spotlight-tile"]');
    var story = document.querySelectorAll('[data-testid="story-tile"]');
    var rows = 0;

    if (spot.length) {
      container.appendChild(sectionTitle('Spotlight (' + spot.length + ')'));
      for (var i = 0; i < spot.length; i++) {
        var a = spot[i].querySelector('a[href*="/spotlight/"]');
        if (a) {
          var mm = (a.getAttribute('href') || '').match(/\/spotlight\/([^?]+)/);
          if (mm) { container.appendChild(row('#' + (i + 1), mm[1], { mono: true, copy: true })); rows++; }
        }
      }
    }
    if (story.length) {
      container.appendChild(sectionTitle('Stories (' + story.length + ')'));
      for (var k = 0; k < story.length; k++) {
        var img = story[k].querySelector('img[data-testid="image-tile-img"]');
        if (img) {
          var id = mediaId(img.getAttribute('src') || '');
          if (id) { container.appendChild(row('#' + (k + 1), id, { mono: true, copy: true })); rows++; }
        }
      }
    }
    if (rows === 0) {
      container.appendChild(statusBox('No Spotlight or Story tiles with extractable IDs in the current DOM. Scroll the profile to load tiles, or Snapchat may have changed the format.', true));
    } else {
      container.appendChild(el('div', 'padding:12px 0 2px 0;color:' + COLORS.textDim + ';font-size:10.5px;line-height:1.5;font-family:' + MONO, 'These IDs identify the media. Reliable timestamp decoding from them is not available, so no time is asserted here.'));
    }
  }
  ...
  function renderPivot(container) {
    var u = username || '';
    container.appendChild(sectionTitle('Snapchat'));
    pivotList(container, [
      { name: 'Public profile', desc: 'Web profile page', url: 'https://www.snapchat.com/add/' + encodeURIComponent(u) },
      { name: 'BACKMOJI', desc: 'Manual Bitmoji history tool', url: 'https://backmoji.myosint.training' }
    ]);
    ...
    pivotList(container, [
      ...
      { name: 'Google', desc: 'Mentions of this handle', url: 'https://www.google.com/search?q=' + encodeURIComponent('"' + u + '" snapchat') },
      { name: 'Wayback Machine', desc: 'Historical snapshots of this profile', url: 'https://web.archive.org/web/*/snapchat.com/add/' + encodeURIComponent(u) }
    ]);
  }
